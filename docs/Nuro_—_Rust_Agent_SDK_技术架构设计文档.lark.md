...
**Nuro** 是一个用 Rust 构建的 Rust Native Agent Runtime & Workflow Framework，对标 LangGraph（图编排）、Google ADK（事件驱动 + 多 Agent 协作）、Claude Agent SDK（Harness + Agent Loop），但充分利用 Rust 的零成本抽象、内存安全、异步并发和类型系统优势，为开发者提供：
<!-- END_BLOCK_3 -->
...
框架采用 Cargo Workspace 的 monorepo 结构，按关注点拆分为独立 crate，用户可按需引入：
<!-- END_BLOCK_15 -->
...
这是整个框架的类型基础，定义所有核心 trait 和数据结构。
<!-- END_BLOCK_21 -->
...
这是框架的核心差异化能力，对标 LangGraph 但提供编译期类型安全。
<!-- END_BLOCK_30 -->
...
**场景**：最基础的交互式 CLI 聊天机器人，带记忆（多轮对话），适合快速验证框架是否跑通。**项目结构**：
<!-- END_BLOCK_95 -->
...
- **开放扩展**：用户可以自定义 Agent 类型而无需修改框架基础层
<!-- END_BLOCK_161 -->
